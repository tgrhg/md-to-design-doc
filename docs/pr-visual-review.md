...
3. workflow が PR コメントに投稿する Preview URL を開く。
4. 画面フッターまたは `version.json` で、レビュー対象のドキュメント版・短縮 SHA・ビルド時刻を確認する。
5. Mermaid、PlantUML、画像、表の表示を視覚確認する。
6. artifact の `design-docs-site` は、権限や Pages 設定で preview URL を使えない場合のフォールバックとして確認する。
...
- PR が更新されるたびに同じ preview subpath を上書きする。
- PR が closed になったら該当 preview subpath を空にする。
