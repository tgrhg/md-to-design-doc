...
3. workflow が `previews/versions.json` に PR preview を登録し、メインページの「公開済みバージョン」からたどれるようにする。
4. workflow が PR コメントに投稿する Preview URL、またはメインページの Version index を開く。
5. 画面フッターまたは `version.json` で、レビュー対象のドキュメント版・短縮 SHA・ビルド時刻を確認する。
6. Mermaid、PlantUML、画像、表の表示を視覚確認する。
