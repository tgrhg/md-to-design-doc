...
const plantUmlMaterialSkin = `skinparam backgroundColor transparent
...
skinparam ActorBackgroundColor #FEF7FF
skinparam ComponentStyle rectangle
skinparam ComponentBorderColor #6750A4
skinparam ComponentBackgroundColor #EADDFF
skinparam DatabaseBorderColor #006A6A
skinparam DatabaseBackgroundColor #D7F7F3
skinparam QueueBorderColor #7D5260
skinparam QueueBackgroundColor #FFD8E4
skinparam ParticipantBorderColor #6750A4
skinparam ParticipantBackgroundColor #EADDFF
skinparam LifeLineBorderColor #CAC4D0
skinparam LifeLineBackgroundColor #F3EDF7
skinparam SequenceBoxBorderColor #CAC4D0
skinparam SequenceBoxBackgroundColor #FFFBFE
skinparam NoteBorderColor #7D5260
skinparam NoteBackgroundColor #FFD8E4
skinparam NoteFontColor #1D1B20
skinparam rectangle {
  BorderColor #6750A4
...
skinparam cloud {
  BorderColor #006A6A
  BackgroundColor #D7F7F3
}`;

function applyPlantUmlTheme(source) {
  if (source.includes('!theme')) return source;
  return source.replace(/@(startuml|startmindmap|startwbs|startsalt|startgantt)([^\n]*)\n/i, (match) => `${match}${plantUmlMaterialSkin}\n`);
}
...
function plantUmlUrl(source) {
  return `https://www.plantuml.com/plantuml/svg/${encodePlantUml(applyPlantUmlTheme(source))}`;
}
  ...
  for (const line of lines) {
    ...
    if (fence && inFence) {
      ...
      if (fenceLang === 'plantuml') {
        html.push(`<figure class="diagram-frame plantuml-frame"><img src="${plantUmlUrl(code)}" alt="PlantUML diagram"><figcaption>PlantUML diagram</figcaption></figure>`);
...
.diagram-frame {
  ...
  border-radius: 28px;
  background:
    linear-gradient(135deg, rgb(234 221 255 / 42%), rgb(215 247 243 / 38%)),
...
.plantuml-frame figcaption::before {
